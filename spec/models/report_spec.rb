...
describe '#get_data_clip' do
  it 'request the json representation of a Heroku dataclip' do
    dummy_report = DummyReport.new(
      'https://dataclips.heroku.com/dataclip',
      '"\k<day> \k<month> \k<year>"'
    )

    return_value = dummy_report.get_data_clip

    expect(return_value).to eq '{"fields":[["string", "Month"],["number", "Count"]],"values":[["01 07 2012",1]]}'
  end
end
...
describe '#get_columns' do
  it 'translates the json response fields to google column declarations' do
    json = '{"fields":["month_string","user_count_number"],"values":[["2012-07-01 00:00:00","1"]]}'
    ...
    expect(return_value).to eq '{"fields":[["string", "Month"],["number", "User Count"]],"values":[["2012-07-01 00:00:00","1"]]}'
  end
end
