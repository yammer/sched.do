Feature: Guest invitees can invite others to an event
  ...
  Scenario: Guest invites another guest via email
    Given someone created an event named "Clown fiesta"
    And "guest@example.com" was invited to the event "Clown fiesta"
    And I am signed in as the guest "guest@example.com"
    When I view the "Clown fiesta" event
    And I invite "dangermouse@example.com" to "Clown fiesta"
